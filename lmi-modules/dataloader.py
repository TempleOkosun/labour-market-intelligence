...
def add_another_data_handler(new_df=None):
    """
...
def json_file_handler(new_df=None):
    ...
    print(msg)
    time.sleep(0.20)
    json_file_name = file_path_finder()
...
def mongodb_load_collection(connection_details):
    """
...
def mongodb_data_import():
   ...
   Returns:
       A pandas.DataFrame is returned
   """
    connection_details = mongodb_conn_details()
    data_from_db = mongodb_load_collection(connection_details)
    ...
    while merge_data_menu:
        ...
        if add_another_data_option == 1:
            ...
            if use_previous_connection == 1:
                ...
                current_connection_details = mongodb_conn_details()
            # Then use the current connection details to load the additional data set
            new_df = mongodb_load_collection(current_connection_details)
